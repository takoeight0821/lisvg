...
// TestLayoutDirections tests all four layout directions
func TestLayoutDirections(t *testing.T) {
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			layouter := NewSimpleLayouter()
			layouter.Direction = LayoutDirection(tt.direction)

			diagram := &Diagram{
				LayoutDirection: tt.direction,
				Nodes: []Node{
					{ID: "A", Label: "Node A"},
					{ID: "B", Label: "Node B"},
					{ID: "C", Label: "Node C"},
			...
			// Check that nodes are positioned correctly based on direction
			nodeA := layout.Nodes["A"]
			nodeB := layout.Nodes["B"]
			nodeC := layout.Nodes["C"]
			...
			case "left-to-right":
				if nodeA.X >= nodeB.X || nodeB.X >= nodeC.X {
					t.Errorf("Left-to-right: Expected A left of B left of C, got A.X=%f, B.X=%f, C.X=%f", nodeA.X, nodeB.X, nodeC.X)
				}
			case "right-to-left":
				if nodeA.X <= nodeB.X || nodeB.X <= nodeC.X {
					t.Errorf("Right-to-left: Expected A right of B right of C, got A.X=%f, B.X=%f, C.X=%f", nodeA.X, nodeB.X, nodeC.X)
				}
			}
		})
	}
...
// TestLayoutDirectionFromDiagram tests that direction is properly read from diagram
func TestLayoutDirectionFromDiagram(t *testing.T) {
	layouter := NewSimpleLayouter()
	...
	// Check that layouter direction was updated
	if layouter.Direction != DirectionLeftToRight {
		t.Errorf("Expected direction %s, got %s", DirectionLeftToRight, layouter.Direction)
	}
	...
	if nodeA.X >= nodeB.X {
		t.Errorf("Left-to-right: Expected A left of B, got A.X=%f, B.X=%f", nodeA.X, nodeB.X)
	}
}

// TestLayoutDirectionEdgePoints tests edge point calculation for different directions
func TestLayoutDirectionEdgePoints(t *testing.T) {
	layouter := NewSimpleLayouter()
	...
	tests := []struct {
		...
		{"TopToBottom", DirectionTopToBottom},
		{"BottomToTop", DirectionBottomToTop},
		{"LeftToRight", DirectionLeftToRight},
		{"RightToLeft", DirectionRightToLeft},
	}

	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			layouter.Direction = tt.direction
			...
			// Check edge points are reasonable
			if len(edge.Points) != 2 {
				t.Errorf("Expected 2 edge points, got %d", len(edge.Points))
			}

			// Verify edge starts near node A and ends near node B
			...
			case DirectionLeftToRight:
				...
				if math.Abs(startPoint.X-(nodeA.X+nodeA.Width/2)) > 0.1 {
					t.Errorf("Left-to-right: Edge should start at right of A")
				}
				if math.Abs(endPoint.X-(nodeB.X-nodeB.Width/2)) > 0.1 {
					t.Errorf("Left-to-right: Edge should end at left of B")
				}
			case DirectionRightToLeft:
				// Should start at left of A and end at right of B
				if math.Abs(startPoint.X-(nodeA.X-nodeA.Width/2)) > 0.1 {
					t.Errorf("Right-to-left: Edge should start at left of A")
				}
				if math.Abs(endPoint.X-(nodeB.X+nodeB.Width/2)) > 0.1 {
					t.Errorf("Right-to-left: Edge should end at right of B")
				}
			}
		})
	}
